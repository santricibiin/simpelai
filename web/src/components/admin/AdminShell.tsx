...
function Shell({ email, siteName, children }: { email: string; siteName: string; children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-dvh">
      <header className="sticky top-0 z-40 border-b border-slate-900/10 bg-offwhite dark:border-white/10 dark:bg-slateDeep-900">
        <div className="flex h-16 items-center gap-3 px-4 sm:px-6">
          <button
            type="button"
            onClick={() => setMobileOpen(true)}
            aria-label="Buka navigasi"
            className="grid h-9 w-9 place-items-center rounded-lg border border-slate-900/10 lg:hidden dark:border-white/10"
          >
